...
**Seed**: `src-tauri/src/db/seed.rs` runs on startup, in a single `BEGIN IMMEDIATE` transaction (falls back to auto-commit), and uses `INSERT OR IGNORE` everywhere — fully idempotent for both fresh installs and upgrades from older schemas. Current **MHP3rd (mhp3rd, game_id 4)**: 1065 items, 60 monsters, 972 weapons, 1111 armor pieces, 378 quests, 263 combines (202 Normal + 61 Alchemy). MHP3rd quests are sourced from `www.mhp3wiki.info` (MHP3攻略データWiki per-rank tables = in-game quest-board order) via `scripts/generate_mhp3rd_quests.py`; `name` = EN objective-derived, `name_original` = real JP quest name (all 378 carry one). `quest_rewards` (1867) resolve JP reward material → `item_id` (fandom `MHP3: Item List` + curated monster-material map); unresolved materials are logged (never orphaned). `backfill_descriptions` and `backfill_item_categories` UPDATE existing rows where `category`/`subcategory` changed (e.g. `Power Juice Material→Consumable/Buff`, `Huskberry Consumable→Ammo/Husk`).
