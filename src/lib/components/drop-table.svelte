      ...
      <div>
        ...
        <div class="space-y-2">
          {#each sec.items as source}
            {@const meta = sourceTypeLabel[source.source_type] ?? {
  ...
  <div class="flex items-center gap-3">
    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-center gap-2 flex-wrap">
        ...
        {#if source.rank}
          <span
            class="text-[10px] px-1.5 py-0.5 rounded border font-semibold {rankStyle[source.rank] ??
              'bg-gray-800 text-gray-300 border-gray-700'}">{source.rank}</span
          >
        {/if}
        {#if detail}
