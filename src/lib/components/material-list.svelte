...
<div class="space-y-3">
  {#if showCraftingCost}
    <div
      class="flex items-center justify-between px-4 py-2.5 rounded-lg border border-[var(--theme-border)] bg-[var(--theme-bg-surface)]"
    >
      <span class="text-xs uppercase tracking-wide text-gray-400">Crafting Cost</span>
  ...
  {#if materials.length === 0}
    <div class="px-4 py-3 rounded-lg border border-dashed border-[var(--theme-border)] text-center">
      <p class="text-sm text-gray-400">No materials required</p>
    </div>
  {:else}
    <div class="space-y-1.5">
      {#each materials as mat, mi (mat.item_id + '-' + mi)}
        <button
          onclick={() => goToItem(mat.item_id)}
          aria-label="Open {mat.item_name}, quantity {mat.quantity}"
          class="w-full flex items-center justify-between gap-3 px-4 min-h-[48px] py-2.5 rounded-lg border border-[var(--theme-border)] bg-[var(--theme-bg-surface)] hover:border-[var(--theme-border-strong)] hover:bg-[var(--theme-bg-elevated)] transition-colors motion-safe:transition-colors motion-reduce:transition-none group focus-visible:outline-none focus-visible:ring-2"
        >
          <div class="flex items-center gap-3 min-w-0">
            ...
            <span
              class="text-sm text-gray-100 group-hover:text-[var(--theme-text-accent)] transition-colors truncate text-left"
            >
              {mat.item_name}
            </span>
          </div>
