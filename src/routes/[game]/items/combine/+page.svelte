...
<div class="max-w-6xl mx-auto">
  <div class="mb-6">
    <div class="flex items-center gap-2">
      ...
      <span
        class="text-xs px-2 py-0.5 rounded-full border bg-[var(--theme-bg-surface)] text-gray-400 border-[var(--theme-border)]"
    ...
    {#if filtered.length > 0}
      <p class="text-[11px] text-gray-600 mt-1">
        💡 All recipes sorted as in the in-game Combination List (Book of Combos 1-5 + Alchemy
        Guide)
      </p>
    {/if}
  </div>
  ...
  {:else}
    <div class="flex flex-wrap gap-2 mb-4 items-center">
      ...
      />
      {#each [{ key: 'all', label: `All (${counts.all})` }, { key: 'normal', label: `Normal (${counts.normal})` }, { key: 'alchemy', label: `⚗️ Alchemy (${counts.alchemy})` }, { key: 'treasure', label: `Treasure (${counts.treasure})` }] as f}
        <button
            ...
            <div class="flex flex-wrap items-center gap-1.5">
              {#each rec.components as comp, i}
                ...
                >
                  ...
                  <span class="text-[10px] font-semibold" style="color: var(--theme-accent);"
                    >x{comp.quantity}</span
                  >
                </button>
              {/each}
              <span class="text-gray-600 text-sm">=</span>
