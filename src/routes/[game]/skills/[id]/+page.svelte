<script lang="ts">
  ...
  const sortedLevels = $derived(skill ? [...skill.levels].sort((a, b) => b.points - a.points) : [])
    ...
    <DetailHeader
      ...
      tags={[
        ...
        ...(skill.armors.length
          ? [
              {
                label: `${skill.armors.length} armor pieces`,
                color: 'bg-[var(--theme-bg-elevated)] text-gray-300 border-[var(--theme-border)]',
              },
            ]
          : []),
        ...(skill.decorations.length
          ? [
              {
                label: `${skill.decorations.length} jewels`,
                color: 'bg-[var(--theme-bg-elevated)] text-gray-300 border-[var(--theme-border)]',
              },
            ]
          : []),
      ...
      {:else}
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <div class="rounded-lg border themed-card overflow-hidden">
            <div
              class="px-4 py-2 bg-emerald-950/30 border-b border-emerald-900/40 flex items-center gap-2"
            >
              <span class="text-emerald-300 text-sm font-semibold">Positive Thresholds</span>
              <span
                class="text-[10px] px-1.5 py-0.5 rounded bg-emerald-900/40 text-emerald-300 border border-emerald-800"
                >{positive.length}</span
              >
            </div>
              ...
              <div class="divide-y divide-gray-800">
                {#each positive as lvl}
                  <div class="px-4 py-3 flex items-center justify-between gap-3">
                    ...
                    <span
                      class="shrink-0 text-xs font-bold px-2.5 py-1 rounded-full bg-emerald-900/40 text-emerald-300 border border-emerald-800"
                      >+{lvl.points}</span
                    >
                  </div>
                {/each}
          ...
          <div class="rounded-lg border themed-card overflow-hidden">
            <div class="px-4 py-2 bg-red-950/30 border-b border-red-900/40 flex items-center gap-2">
              ...
              <span
                class="text-[10px] px-1.5 py-0.5 rounded bg-red-900/40 text-red-300 border border-red-800"
              ...
              <div class="divide-y divide-gray-800">
                {#each negative as lvl}
                  <div class="px-4 py-3 flex items-center justify-between gap-3">
                    ...
                    <span
                      class="shrink-0 text-xs font-bold px-2.5 py-1 rounded-full bg-red-900/40 text-red-300 border border-red-800"
                      >{lvl.points}</span
                    >
                  </div>
                {/each}
        ...
        <div class="grid grid-cols-1 gap-3">
          {#each skill.decorations as deco}
            ...
            >
              <!-- Header -->
              <button
                onclick={() => openDecoration(deco.id)}
                class="w-full text-left px-4 py-3 flex items-start justify-between gap-3 hover:bg-[var(--theme-bg-elevated)]/40 transition-colors"
              >
                <div class="min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    <span
                      class="text-xs px-2 py-0.5 rounded-full font-bold border {deco.skill_points >=
                      0
                        ? 'bg-emerald-900/30 text-emerald-300 border-emerald-800'
                        : 'bg-red-900/30 text-red-300 border-red-800'}"
                    >
                      {skill.name}
                      {deco.skill_points > 0 ? '+' : ''}{deco.skill_points}
                    </span>
                    {#if deco.secondary_skill_name}
                      <span
                        class="text-xs px-2 py-0.5 rounded bg-[var(--theme-bg-elevated)] text-gray-400 border border-[var(--theme-border)]"
                      >
                        ...
                        <span
                          ...
                          >{(deco.secondary_points ?? 0) > 0
                            ? '+'
                            : ''}{deco.secondary_points}</span
                        >
                      </span>
                    {/if}
                  </div>
                  <div class="flex flex-wrap gap-1.5 mt-2">
                    <span
                      class="text-[10px] uppercase tracking-wide px-2 py-0.5 rounded bg-[var(--theme-bg-elevated)] text-gray-400 border border-[var(--theme-border)]"
                      >{deco.unlock}</span
                    >
                    <span
                      class="text-[10px] px-2 py-0.5 rounded bg-yellow-900/20 text-yellow-300 border border-yellow-800"
              ...
              <div class="px-4 pb-3">
                    ...
                    {#each deco.materials as mat}
                      <button
                        ...
                        class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs border transition-colors
                          {mat.item_id
                          ? 'bg-[var(--theme-bg-elevated)] text-gray-200 border-[var(--theme-border)] hover:border-[var(--theme-border-strong)] hover:bg-[var(--theme-bg-surface)] cursor-pointer'
                          : 'bg-gray-800 text-gray-400 border-gray-700 cursor-default'}"
                        title={mat.item_id
                          ? 'View item source (monster drops, gathering)'
                          : 'Material not in item DB - faithful name'}
                      >
                        <span class="font-medium">{mat.item_name}</span>
                        <span class="px-1.5 py-0.5 rounded bg-black/20 text-[11px] font-bold"
                          >x{mat.quantity}</span
                        >
                      </button>
                    {/each}
                ...
                <p class="text-[10px] text-gray-500 mt-2 leading-relaxed">
                  ...
                  (High), <span class="text-gray-300">Battlefield Jewel</span> (G),
                  <span class="text-gray-300">LapisLazuli Jewel</span> (G). Each material links to its
                  monster drop source.
                </p>
              </div>
            </div>
