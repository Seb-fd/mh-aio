<script lang="ts">
  ...
  async function load(id: number) {
    try {
      ...
      mantles = all.filter((m) => m.tool_type === 'mantle')
      error = null
    } catch (e) {
      error = String(e)
    } finally {
...
<div class="max-w-6xl mx-auto">
  <div class="flex flex-wrap gap-2 mb-4 items-center">
    ...
    />
    <span class="text-xs text-gray-500">{filtered.length} / {mantles.length} mantles</span>
  </div>
  {#if loading}
    <div class="border rounded-lg p-8 text-center themed-card">
      <p class="text-gray-400">Loading mantles...</p>
    </div>
  {:else if error}
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
      {#each filtered as m (m.id)}
        <button onclick={() => open(m.id)} class="text-left">
          <Card class="p-4 border transition-all cursor-pointer themed-card hover:scale-[1.01]">
            <div class="flex items-start gap-3">
              ...
              <div class="min-w-0 flex-1">
                ...
                <p class="text-[11px] text-gray-500 mt-0.5 line-clamp-2 leading-tight">
                  {m.effect}
                </p>
                <p class="text-[11px] mt-1 flex gap-2">
                  <span class="text-gray-400">⏱ {m.duration_sec ?? '-'}s</span>
                  <span class="text-gray-500"
                    >↻ {m.cooldown_sec ?? '-'}s → {m.cooldown_upgraded_sec ?? '-'}s (+)</span
                  >
                </p>
                {#if m.slots && m.slots !== '---'}
              ...
              <span
                class="text-[10px] px-2 py-0.5 rounded bg-[var(--theme-bg-elevated)] border border-[var(--theme-border)] text-gray-400 shrink-0"
                >Mantle</span
              >
            </div>
            {#if m.upgrade_quest}
              <p class="text-[10px] text-emerald-400/80 mt-2 truncate">★ {m.upgrade_quest}</p>
            {/if}
          </Card>
        </button>
      {/each}
