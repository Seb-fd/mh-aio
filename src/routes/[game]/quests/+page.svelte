<script lang="ts">
  ...
  const filtered = $derived(quests.filter((q) => q.hub === hubFilter))
      ...
      {:else}
        <div class="space-y-2">
          {#each starGroups as group}
            <div
              class="rounded-lg border border-[var(--theme-border)] bg-[var(--theme-bg-surface)] overflow-hidden"
            >
              <button
              ...
              >
                <span
                  class="text-[10px] text-gray-400 transition-transform {isOpen(group.stars)
                    ? 'rotate-90'
                    : ''}">▶</span
                >
                        ...
                        <div class="flex items-start justify-between gap-3">
                          <div class="min-w-0 flex-1">
                            <div class="flex items-center gap-2 mb-1 flex-wrap">
                              ...
                              {#if quest.stars}
                                <span
                                  class="text-[10px] px-1.5 py-0.5 rounded bg-[var(--theme-bg-elevated)] text-gray-300 border border-[var(--theme-border)]"
                                  >{starsLabel(quest)}</span
                                >
                              {/if}
                              {#if quest.is_key_quest}
                                <span
                                  class="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded bg-yellow-500/10 text-yellow-500 border border-yellow-500/30"
                                >
                                  Key
                                </span>
                              {/if}
                              {#if quest.is_urgent}
                                <span
                                  class="text-[10px] uppercase tracking-wide px-1.5 py-0.5 rounded bg-red-500/10 text-red-400 border border-red-500/30"
                                >
                                  Urgent
                                </span>
                              {/if}
