...
fn skill_points_from_str(skills: &Option<String>, skill_name_to_id: &HashMap<String, i32>) -> HashMap<i32, i32> {
...
fn load_skill_name_map(conn: &Connection, game_id: i32) -> rusqlite::Result<HashMap<String, i32>> {
    ...
    let rows = stmt.query_map([game_id], |row| Ok((row.get::<_, i32>(0)?, row.get::<_, String>(1)?)))?;
    let mut map = HashMap::new();
    for r in rows {
        let (id, name) = r?;
...
fn armor_matches_query(armor: &Armor, q: &QueryInternal, danger_skills: &HashSet<i32>, max_slots: usize) -> (bool, Option<i32>, bool) {
...
fn is_better_armor(a: &Armor, b: &Armor, rel: &[i32]) -> bool {
    ...
    let a_no = a.skill_points.iter().all(|(k, v)| !rel.contains(k) || *v <= 0);
    let b_no = b.skill_points.iter().all(|(k, v)| !rel.contains(k) || *v <= 0);
    if a_no && b_no {
        return if a.defence == b.defence { a.rarity > b.rarity } else { a.defence > b.defence };
    }
    if a.is_torso_inc && b.is_torso_inc {
        return if a.rarity == b.rarity { a.defence > b.defence } else { a.rarity > b.rarity };
...
fn is_better_decoration(a: &Decoration, b: &Decoration, _rel: &[i32]) -> bool {
    ...
    if b_worse && (!a_worse || a.abilities[1].1 * (b.slot_size as i32) < b.abilities[1].1 * (a.slot_size as i32)) {
        return true;
    }
    false
}
...
fn get_relevant_data(conn: &Connection, input: &AssQueryInput) -> rusqlite::Result<QueryInternal> {
    ...
    for deco in all_decos.drain(..) {
        ...
        if rel_ids.contains(&primary) {
            ...
            for i in 0..q.rel_decorations.len() {
                if deco.id == q.rel_decorations[i].id { inserted=true; break; }
    ...
    for a in all_armors {
        ...
        if idx < NUM_ARMOR_TYPES { grouped[idx].push(a); }
    }
    ...
    for slot in 0..NUM_ARMOR_TYPES {
        ...
        for armor in list {
            let (matches, _danger, _no_skills) = armor_matches_query(&armor, &q, &danger_skills, max_slots);
            if matches {
                ...
                if should_add {
                    for idx in to_remove.iter().rev() { q.rel_armor[slot].remove(*idx); }
...
fn create_equivalences(q: &QueryInternal) -> Vec<Vec<ArmorEquivalence>> {
    ...
    for slot in 0..NUM_ARMOR_TYPES {
        ...
        for armor in &q.rel_armor[slot] {
            ...
            if need_new {
                list.push(ArmorEquivalence::from_armor(armor.clone(), &q.rel_skill_ids));
            }
        }
        armor_eq[slot] = list;
    }
...
fn eq_matches(eq: &ArmorEquivalence, armor: &Armor, rel: &[i32]) -> bool {
    if eq.num_slots != armor.num_slots { return false; }
    if eq.torso_inc { return armor.is_torso_inc; } else if armor.is_torso_inc { return false; }
    if eq.no_skills { return armor.skill_points.iter().all(|(k, v)| !rel.contains(k) || *v <=0); }
    else if armor.skill_points.iter().all(|(k, v)| !rel.contains(k) || *v <=0) { return false; }
    for (sid, pts) in &eq.abilities {
        if armor.skill_points.get(sid).copied().unwrap_or(0) != *pts { return false; }
    }
    true
}
...
impl EquivalenceSolution {
    fn matches_query(&mut self, q: &QueryInternal) -> bool {
        ...
        for (idx, sid) in q.rel_skill_ids.iter().enumerate() {
            for req in &q.skills {
                if req.skill_id == *sid { desired[idx] = req.points_required; break; }
            }
        }
        ...
        self.calculate_equivalence_info(&q.ability_index, &self.armor_eq[0].clone(), &mut skill_points, false);
        self.calculate_equivalence_info(&q.ability_index, &self.armor_eq[2].clone(), &mut skill_points, false);
        self.calculate_equivalence_info(&q.ability_index, &self.armor_eq[3].clone(), &mut skill_points, false);
        self.calculate_equivalence_info(&q.ability_index, &self.armor_eq[4].clone(), &mut skill_points, false);
        self.calculate_equivalence_info(&q.ability_index, &self.armor_eq[1].clone(), &mut skill_points, true);
        let body_slots = self.armor_eq[1].num_slots;
        if body_slots <4 { self.slots_spare[body_slots] = self.slots_spare[body_slots].saturating_sub(1); }
        ...
        for i in 0..desired.len() {
            if skill_points[i] < desired[i] { return false; }
        }
        true
    }
    fn calculate_equivalence_info(&mut self, ability_map: &HashMap<i32, usize>, eq: &ArmorEquivalence, skills: &mut Vec<i32>, torso: bool) {
        self.torso_multiplier += if eq.torso_inc {1} else {0};
        if eq.num_slots <4 { self.slots_spare[eq.num_slots] +=1; }
    ...
    fn calculate_decorations(&mut self, q: &QueryInternal, skill_points: &mut Vec<i32>, desired: &Vec<i32>) {
        let mut body_slots = [0usize;4];
        let mut temp_slots = [0usize;4];
        if q.weapon_slots <4 { self.slots_spare[q.weapon_slots] +=1; }
        for i in 1..4 { temp_slots[i]= self.slots_spare[i]; }
        if self.torso_slots_spare <4 { body_slots[self.torso_slots_spare] +=1; }

        // 3-slot decos
        self.add_decorations_23(&q.ability_index, &q.sorted_decorations[3].clone(), 3, self.torso_multiplier, &mut body_slots[3], skill_points, desired);
        body_slots[1] += body_slots[3];
        body_slots[2] += body_slots[3];
        body_slots[3]=0;
        self.add_decorations_23(&q.ability_index, &q.sorted_decorations[3].clone(), 3, 1, &mut temp_slots[3], skill_points, desired);
        ...
        self.add_decorations_23(&q.ability_index, &q.sorted_decorations[2].clone(), 2, self.torso_multiplier, &mut body_slots[2], skill_points, desired);
        body_slots[1] += 2*body_slots[2];
        body_slots[2]=0;
        {
            let before = temp_slots[2];
            self.add_decorations_23(&q.ability_index, &q.sorted_decorations[2].clone(), 2, 1, &mut temp_slots[2], skill_points, desired);
            ...
            } else { self.slots_spare[2] -= used; }
        }
        temp_slots[1] += 2*temp_slots[2];
        temp_slots[2]=0;
        self.add_decorations_1(&q.ability_index, &q.sorted_decorations[1].clone(), self.torso_multiplier, &mut body_slots[1], skill_points, desired);
        {
            let before = temp_slots[1];
            self.add_decorations_1(&q.ability_index, &q.sorted_decorations[1].clone(), 1, &mut temp_slots[1], skill_points, desired);
    ...
    fn add_decorations_1(&mut self, ability_map: &HashMap<i32, usize>, decos: &Vec<Decoration>, multiplier: usize, num_slots: &mut usize, skill_points: &mut Vec<i32>, desired: &Vec<i32>) {
        if *num_slots==0 || decos.is_empty() { return; }
        let mut i=0usize;
            ...
            if let Some(&idx)=ability_map.get(&ap.0) {
                ...
                if amt>0 && cur < need && cur+amt <= need+1 {
                    ...
                    *num_slots-=1;
                    ...
                    if i==0 { i = decos.len()-1; } else { i-=1; }
                }
            }
            i+=1;
            if i>=decos.len() { i=0; looped=true; }
            if looped && i==0 { break; }
        }
    }
    fn add_decorations_23(&mut self, ability_map: &HashMap<i32, usize>, decos: &Vec<Decoration>, _size: usize, multiplier: usize, num_slots: &mut usize, skill_points: &mut Vec<i32>, desired: &Vec<i32>) {
        if *num_slots==0 || decos.is_empty() { return; }
        ...
        // pick best by score
        let mut best_idx=0;
        let mut best_score = get_score(&candidates[0].0, ability_map, skill_points, candidates[0].2);
        ...
        *num_slots-=1;
        if *num_slots>0 {
            self.add_decorations_23(ability_map, decos, _size, multiplier, num_slots, skill_points, desired);
        }
    }
}

fn get_score(deco: &Decoration, ability_map: &HashMap<i32, usize>, skill_points: &Vec<i32>, actual_good: i32) -> (i32,i32) {
    let point_score = [0,1,2,3,4,3];
    let idx = actual_good as usize; let good = if idx < point_score.len() { point_score[idx] } else {0};
    let bad = if deco.abilities.len()==1 {0} else {
        if let Some(&idx)=ability_map.get(&deco.abilities[1].0) { skill_points[idx] } else {0}
    };
    (good as i32, bad)
}
...
struct FinalSolution {
    ...
    fire: i32, ice: i32, water: i32, thunder: i32, dragon: i32,
    defence: i32, rarity: i32, difficulty: i32,
}
...
impl FinalSolution {
    fn from_equivalence(es: &EquivalenceSolution) -> Self {
        ...
        for i in 1..4 { slots_spare[i]= es.slots_spare[i] as i32; total+= (i as i32)*slots_spare[i]; }
    ...
    fn calculate_extra(&mut self, q: &QueryInternal, skill_id_to_name: &HashMap<i32,String>, torso_eq: &ArmorEquivalence) {
        ...
        for armor in &self.armors {
            ...
            let mult = if is_body { self.torso_multiplier as i32 } else {1};
            for (sid, pts) in &armor.skill_points {
    ...
    fn check_bad(&mut self, q: &QueryInternal, deco_map: &HashMap<i32, Vec<Decoration>>) -> bool {
        ...
        for (sid, _name, pts) in &self.extra_skills {
            ...
            if *pts <= -10 { bad.push(*sid); }
        }
        if bad.is_empty() { return true; }
        if self.total_slots_spare >0 {
            if self.fix_bad_skills(&bad, q, deco_map) { return true; }
        }
        self.reorder_gems(&bad, q, deco_map)
    }
    fn fix_bad_skills(&mut self, bad: &Vec<i32>, q: &QueryInternal, deco_map: &HashMap<i32, Vec<Decoration>>) -> bool {
        for sid in bad {
            if !self.fix_bad_skill(*sid, q, deco_map) { return false; }
        }
        true
    }
    fn fix_bad_skill(&mut self, sid: i32, q: &QueryInternal, deco_map: &HashMap<i32, Vec<Decoration>>) -> bool {
        ...
        while torso_slots>0 {
            if let Some(deco)=get_best_decoration(sid, torso_slots as usize, q, deco_map) {
                ...
                if is_detrimental(&deco, q) { return false; }
                if self.add_decoration(deco.clone()) { return true; }
                torso_slots -= sz;
            } else { break; }
        }
        let mut max_slots=3;
        while max_slots>0 && self.slots_spare[max_slots]==0 { max_slots-=1; }
        ...
        if let Some(deco)=get_best_decoration(sid, max_slots, q, deco_map) {
            ...
            if self.slots_spare[dsz]==0 {
                if deco.slot_size==1 {
                    if self.slots_spare[2]==0 && self.slots_spare[3]==0 { return false; }
                    if self.slots_spare[2]==0 { self.slots_spare[3]-=1; self.slots_spare[2]+=1; }
                    else { self.slots_spare[2]-=1; self.slots_spare[1]+=1; }
            ...
            if self.slots_spare[deco.slot_size as usize]>0 {
                ...
                if self.add_decoration(deco) { return true; }
            }
        }
        false
    }
    ...
    fn calculate_data(&mut self) {
        self.fire=0; self.ice=0; self.thunder=0; self.water=0; self.dragon=0; self.defence=0; self.rarity=0; self.difficulty=0;
        for a in &self.armors {
            self.fire+=a.fire; self.ice+=a.ice; self.thunder+=a.thunder; self.water+=a.water; self.dragon+=a.dragon;
            self.defence+=a.defence; self.rarity+=a.rarity;
            // difficulty simplified: rarity>4 counts
            self.difficulty += if a.rarity>=8 {3} else if a.rarity>=6 {2} else if a.rarity>=4 {1} else {0};
        }
        self.total_slots_spare = self.slots_spare[1]+self.slots_spare[2]*2+self.slots_spare[3]*3 + self.torso_slots_spare as i32;
    }
}

fn is_detrimental(deco: &Decoration, q: &QueryInternal) -> bool {
    if deco.abilities.len()<2 { return false; }
    for req in &q.skills {
        if req.skill_id == deco.abilities[1].0 { return true; }
    }
    false
}
...
fn get_best_decoration(ability: i32, max_slots: usize, q: &QueryInternal, map: &HashMap<i32, Vec<Decoration>>) -> Option<Decoration> {
    ...
    let rel = vec![ability];
    for deco in list {
        if deco.slot_size > max_slots { continue; }
        if let Some(b)= &best {
            if is_better_decoration(deco, b, &rel) && !is_better_decoration(b, deco, &rel) { best=Some(deco.clone()); }
        } else { best=Some(deco.clone()); }
    }
    best
}
...
pub fn search(conn: &Connection, input: AssQueryInput) -> Result<Vec<AssSolutionView>, String> {
    ...
    if q.skills.is_empty() { return Err("Selecciona al menos 1 habilidad".to_string()); }
    // Check each slot has at least one armor
    for i in 0..NUM_ARMOR_TYPES {
        if q.rel_armor[i].is_empty() { return Err(format!("No hay armaduras relevantes para slot {}", i)); }
    }
    let armor_eq = create_equivalences(&q);
    for i in 0..NUM_ARMOR_TYPES {
        if armor_eq[i].is_empty() { return Err(format!("No hay equivalencias para slot {}", i)); }
    }
    // Build decoration map for bad-fix
    let mut deco_map: HashMap<i32, Vec<Decoration>> = HashMap::new();
    for d in &q.rel_decorations {
        deco_map.entry(d.abilities[0].0).or_default().push(d.clone());
    }
    // skill id -> name
    let mut skill_names: HashMap<i32,String>=HashMap::new();
    {
        let mut stmt = conn.prepare("SELECT id, name FROM skills WHERE game_id = ?1").map_err(|e| e.to_string())?;
        let rows = stmt.query_map([input.game_id], |row| Ok((row.get::<_,i32>(0)?, row.get::<_,String>(1)?))).map_err(|e| e.to_string())?;
        for r in rows { let (id,n)=r.map_err(|e| e.to_string())?; skill_names.insert(id,n); }
    }
            ...
            for arms_eq in &arms_eq_list {
                for waist_eq in &waist_eq_list {
                    for legs_eq in &legs_eq_list {
                        if solutions.len() >= MAX_LIMIT { break 'outer; }
                        let mut es = EquivalenceSolution{ armor_eq: vec![head_eq.clone(), body_eq.clone(), arms_eq.clone(), waist_eq.clone(), legs_eq.clone()], decorations: Vec::new(), multipliers: Vec::new(), slots_spare:[0;4], torso_slots_spare:0, torso_multiplier:1 };
                        if !es.matches_query(&q) { continue; }
                                ...
                                for a in &arms_eq.armors {
                                    for w in &waist_eq.armors {
                                        for l in &legs_eq.armors {
                                            ...
                                            sol.armors = vec![h.clone(), b.clone(), a.clone(), w.clone(), l.clone()];
                                            sol.calculate_extra(&q, &skill_names, body_eq);
                                            if !sol.check_bad(&q, &deco_map) { continue; }
                                            sol.calculate_data();
                                            solutions.push(sol);
                                        }
                                    }
    ...
    for sol in solutions.iter().take(MAX_LIMIT) {
        let armors: Vec<AssArmorRef> = sol.armors.iter().map(|a| AssArmorRef{
            id: a.id, name: a.name.clone(), slot_type: a.slot_type.clone(), rarity: Some(a.rarity), defense_base: Some(a.defence), slots: Some(a.num_slots.to_string()), skills: Some(a.skill_points.iter().map(|(k,v)| format!("{} {}", skill_names.get(k).unwrap_or(&k.to_string()), v)).collect::<Vec<_>>().join(", "))
        }).collect();
        // group decorations by id
        let mut counts: HashMap<i32, i32>=HashMap::new();
        for d in &sol.decorations { *counts.entry(d.id).or_insert(0)+=1; }
        let mut decos: Vec<AssDecorationRef>=Vec::new();
        for (id,cnt) in counts {
            if let Some((name,sn,sp,sn2,sp2,slot)) = deco_info.get(&id) {
                decos.push(AssDecorationRef{ id, name: name.clone(), slot_size: *slot, skill_name: sn.clone(), skill_points: *sp, secondary_skill_name: sn2.clone(), secondary_points: *sp2, count: cnt });
            }
        }
        let extra: Vec<String> = sol.extra_skills.iter().filter(|(sid,_,_)| !input.skills.iter().any(|r| r.skill_id==*sid)).map(|(_,n,pts)| format!("{} {}", n, pts)).collect();
        views.push(AssSolutionView{
            armors, decorations: decos, extra_skills: extra,
            defense: sol.defence, fire_res: sol.fire, water_res: sol.water, thunder_res: sol.thunder, ice_res: sol.ice, dragon_res: sol.dragon,
            rarity: sol.rarity, difficulty: sol.difficulty, slots_spare: sol.total_slots_spare,
            slots_spare_detail: vec![sol.slots_spare[0], sol.slots_spare[1], sol.slots_spare[2], sol.slots_spare[3]],
        });
    }
    Ok(views)
}
